    ...
    try {            
      ...
      upload.array('images', 5)(req, res, async (err) => {
        ...
        const updateData = {
          ...
          desktop_environment: desktop_environment || existingConfig.desktop_environment,
        ...
        let imageUrls = existingConfig.image_url ? JSON.parse(existingConfig.image_url) : [];
        ...
        if (req.files && req.files.length > 0) {
          ...
          const uploadImage = await Promise.all(req.files.map(file => uploadImage(file)));
          imageUrls = [...imageUrls, ...uploadImage]
